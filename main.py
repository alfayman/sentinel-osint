...
# --- 1. إعداد الصفحة والديزاين (Diamond Cyber-UI) ---
...
st.markdown("""
    ...
    @keyframes eyePulse { 0%, 100% { transform: rotate(45deg) scale(1); } 50% { transform: rotate(45deg) scale(1.05); } }
    ...
    .adsense-box { background: rgba(255, 255, 255, 0.02); border: 1px dashed #333; text-align: center; padding: 10px; color: #444; margin: 10px 0; }
    </style>
...
with tab1:
        ...
        if uploaded_file:
            ...
            if st.button(i18n["scan"]):
                with st.spinner("Analyzing..."):
                    try:
                        # استخدام gemini-1.5-flash الصحيح لحل مشكل 404
    ...
    with col_r:
        ...
        st.markdown("<div class='adsense-box' style='height:300px;'>AdSense Vertical</div>", unsafe_allow_html=True)
